...
export function ApproveWorkReportButton({ token }: { token: string }) {
  const [pending, startTransition] = useTransition();
  const [result, setResult] = useState<"ok" | string | null>(null);

  if (result === "ok") {
    return <p className="text-center text-sm font-semibold text-primary">承認しました。</p>;
  }
  if (result) {
